...
__all__ = ['CurrencyConverter', 'DayCountConverter', 'SwaptionTypeConverter', 'SwaptionStrategyTypeConverter',
           'CapFLoorTypeConverter', 'CapFLoorStrategyTypeConverter', 'SettlementMethodConverter']
...
class SwaptionTypeConverter(DictConverter[SwaptionStrategy]):
    grammar = GRAMMAR
    name = 'SWAPTION_TYPE'
    mapping = bidict({'R': SwaptionStrategy.RECEIVER,
...
class CapFLoorTypeConverter(DictConverter[CapFloorStrategy]):
    grammar = GRAMMAR
    name = 'CAP_FLOOR_TYPE'
    mapping = bidict({'C': CapFloorStrategy.CAP,
...
class SettlementMethodConverter(DictConverter[SettlementMethod]):
    ...
    mapping = bidict({'CASH': SettlementMethod.CASH_SETTLED_ISDA,
